...
def train_model(era5_dataset, ukesm_dataset, info, num_epochs):
    ...
    for fold, (train_indices, val_indices) in enumerate(kf.split(train_dataset)):
        ...
        for idx, (_, label, time) in enumerate(train_loader):
            if idx < 100:
                print(f"idx: {idx}")
                print(f"label: {label}")
                print(f"time: {time}")

        for idx, (_, label, time) in enumerate(val_loader):
            print(f"idx: {idx}")
            print(f"label: {label}")
            print(f"time: {time}")
    ...
    #         print(
    #             (
    #                 torch.bincount(epoch_predictions.long()),
    #                 torch.bincount(epoch_labels.long()),
    #             )
    #         )
    ...
    #                     print(
    #                         "false_positives: " + str(torch.sum(false_positives).item())
    #                     )
    #                     print(
    #                         "false_negatives: " + str(torch.sum(false_negatives).item())
    #                     )

    #                     for idx, (fp, fn) in enumerate(
    #                         zip(false_positives, false_negatives)
    #                     ):
    #                         t_str = datetime(1900, 1, 1) + timedelta(hours=int(t[idx]))
    #                         if fp.item():
    #                             img = get_image(inputs[idx], t_str)
    #                             validation_writer.add_image(
    #                                 f"false-positive/{t_str.strftime('%Y-%m-%d')}",
    #                                 img,
    #                                 epoch,
    #                             )
    #                         if fn.item():
    #                             img = get_image(inputs[idx], t_str)
    #                             validation_writer.add_image(
    #                                 f"false-negative/{t_str.strftime('%Y-%m-%d')}",
    #                                 img,
    #                                 epoch,
    #                             )
...
train_model(era5_dataset, ukesm_dataset, "debug", NUM_EPOCHS)
